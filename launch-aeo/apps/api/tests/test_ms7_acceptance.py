...
_REPO_ROOT = _LAUNCH_AEO_ROOT.parent

_REQUIRED_MS7_ARTIFACTS = [
    "pilot/yuanzheng-sku-testset.json",
...
_MS7_DOC_ARTIFACTS = [
    "docs/pilot/yuanzheng-sku-testset.md",
...
def test_ms7_required_code_artifacts_exist() -> None:
    for relative_path in _REQUIRED_MS7_ARTIFACTS:
        path = _LAUNCH_AEO_ROOT / relative_path
...
def test_ms7_testset_has_20_yuanzheng_skus() -> None:
    items = load_pilot_testset(_LAUNCH_AEO_ROOT / "pilot" / "yuanzheng-sku-testset.json")
    assert len(items) == 20
...
def test_ms7_s7_deliverable_tests_present() -> None:
    tests_dir = _LAUNCH_AEO_ROOT / "apps" / "api" / "tests"
    for name in (
