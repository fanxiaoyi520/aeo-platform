...
def test_extract_docx_text() -> None:
    content = _make_docx_bytes("LAUNCH X431 PRO diagnostic scanner")
    text = extract_docx_text(content)
    assert "LAUNCH X431 PRO" in text
...
def test_extract_text_from_bytes_docx() -> None:
    content = _make_docx_bytes("OBD2 scanner keywords")
...
def test_load_docx_file_via_loader(tmp_path: Path) -> None:
    ...
    docx_path = products / "launch-x431.docx"
    docx_path.write_bytes(_make_docx_bytes("LAUNCH X431 product parameters"))

    doc = load_file(docx_path, tmp_path)
    assert doc is not None
    assert doc.category == "product"
    assert "LAUNCH X431" in doc.content
