...
_TESTSET = _ROOT / "pilot" / "yuanzheng-sku-testset.json"
...
def test_compute_adoption_score_exact_match() -> None:
    generated = {"title": "LAUNCH X431 Pro Scanner"}
    final_output = {"title": "LAUNCH X431 Pro Scanner"}
    assert compute_adoption_score(generated, final_output) == 1.0


def test_extract_pilot_record_hitl_first_try() -> None:
    item = {"id": "YZ-001", "sku": "X431-PRO", "platform": "amazon", "market": "US"}
