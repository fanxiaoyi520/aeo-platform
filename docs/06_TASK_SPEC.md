...
> **⚠️ 必须项：Spec 无开源调研 → 不得开工。**

| 级别 | 何时 | Spec **必须**包含 |
|------|------|-------------------|
| **L1** | bugfix、纯文档/配置、<30min 小改、adapter 薄封装 | **开源参考：** 2～3 行（项目名 + 采纳方式），**禁止留空** |
| **L2** | 新包、新 Agent 能力、外部系统集成、新 pipeline | **开源调研：** Top 3 对比表 + 采纳决策，**禁止留空** |

**L1 示例（必填）：**
```
开源：L1 — 参考 FastAPI Depends() 依赖注入模式；项目内已有 get_db_session 先例，复用。
```
...
**开源（必填）：** L1 / L2（见 §开源优先）— L1 写 2~3 行参考；L2 写 Top 3 对比表 + 采纳决策。**禁止留空或写「无」。**
