...
QUESTIONS: list[tuple[str, str, list[str]]] = [
    ...
    ("Q12", "消费电子类目应写清什么技术参数？", ["蓝牙", "防水", "IPX"]),
    ("Q13", "耳机类产品应写清什么续航信息？", ["续航", "充电"]),
    ("Q14", "消费电子 Listing 应避免什么绝对化表述？", ["所有设备", "绝对"]),
    ("Q15", "标题是否禁止特殊符号堆砌？", ["禁止", "特殊符号"]),
    ("Q16", "五点描述应突出什么？", ["功能", "场景", "差异化"]),
    ("Q17", "Bullet 结尾是否使用标点？", ["不用", "标点"]),
    ("Q18", "Search Terms 单位是字符还是 bytes？", ["bytes"]),
    ("Q19", "电池容量是否应在 Listing 中写清？", ["电池", "容量"]),
...
def main() -> None:
    ...
    if rate < 0.8:
        raise SystemExit(
            f"Relevance {rate:.0%} below 80% threshold (hash embeddings are approximate)"
        )
    if p95_ms >= 500:
