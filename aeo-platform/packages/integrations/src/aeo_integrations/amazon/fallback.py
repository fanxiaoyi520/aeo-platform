...
class FallbackWrapper:
    ...
    def data_source(self) -> str:
        return "spapi-degraded" if self._degraded else "spapi"
        ...
        **kwargs: Any,
        ...
        for attempt in range(self._max_retries + 1):
            try:
                ...
                if self._degraded:
                    logger.info("SP-API recovered from degradation for %s", method_name)
                    self._degraded = False
                return result
                ...
                if attempt < self._max_retries:
                    ...
                    logger.warning(
                        "SP-API %s attempt %d failed (%s), retrying in %.1fs",
                        method_name,
                        attempt + 1,
                        type(exc).__name__,
                        delay,
        ...
        self._degraded = True
        logger.warning(
            "SP-API %s failed after %d attempts (%s), falling back to mock",
            method_name,
            self._max_retries + 1,
