"""Facebook Ads — Protocol + MockAdapter + Factory."""
...
class FacebookAdsClient(Protocol):
    """Read-only Facebook Ads API client."""
...
class MockFacebookAdsAdapter:
    """Mock Facebook Ads adapter using local JSON fixtures."""
    ...
    def _load_campaigns(self) -> dict[str, FacebookAdCampaign]:
        ...
        campaigns = [FacebookAdCampaign.model_validate(c) for c in raw["campaigns"]]
        self._campaign_cache = {c.campaign_id.upper(): c for c in campaigns}
        return self._campaign_cache

    def _load_snapshots(self) -> list[FacebookAdSpendSnapshot]:
        if self._snapshot_cache is not None:
            return self._snapshot_cache
        raw = json.loads(self._fixture_path.read_text(encoding="utf-8"))
        self._snapshot_cache = [
            FacebookAdSpendSnapshot.model_validate(s) for s in raw["spend_snapshots"]
        ]
        return self._snapshot_cache
