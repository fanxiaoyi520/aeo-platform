...
def resolve_project_path(relative: str) -> Path:
    """Resolve path relative to aeo-platform monorepo root."""
    here = Path(__file__).resolve()
    for parent in here.parents:
        if (parent / "pyproject.toml").exists() and (parent / "knowledge").exists():
            return (parent / relative).resolve()
        if (parent / "pyproject.toml").exists() and parent.name == "aeo-platform":
            return (parent / relative).resolve()
    # fallback: packages/rag/src/aeo_rag -> aeo-platform
